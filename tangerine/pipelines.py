...
class TtangerinePipeline:

    # Database setup
    conn = sqlite3.connect('tangerine.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `tangerine`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM tangerine WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO tangerine (date, title, link, content)"
